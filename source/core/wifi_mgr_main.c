...
{
    int ret = RETURN_OK;
    wifi_mgr_t *wifimgr = get_wifimgr_obj();
    ...
    ret = wifi_hal_getHalCapability(&wifimgr->hal_cap);
...
int init_wifimgr()
{
    ...
    memset(wifimgr->channel_change_in_progress, 0, sizeof(wifimgr->channel_change_in_progress));
    ...
    for (itr=0; itr < (int)getNumberRadios(); itr++) {
        init_global_radio_config(&wifimgr->radio_config[itr], itr);
    }
    ...
    //Init csi_data_queue
    if (wifimgr->csi_data_queue == NULL) {
        wifimgr->csi_data_queue = queue_create();
    }

    if (wifimgr->stats_config_map == NULL) {
        wifimgr->stats_config_map = hash_map_create();
    }

    if (wifimgr->steering_config_map == NULL) {
        wifimgr->steering_config_map = hash_map_create();
    }

    if (wifimgr->steering_client_map == NULL) {
        wifimgr->steering_client_map = hash_map_create();
    }


    if (wifimgr->vif_neighbors_map == NULL) {
        wifimgr->vif_neighbors_map = hash_map_create();
    }
...
int start_wifimgr()
{
    ...
    if (start_wifi_ctrl(&wifimgr->ctrl) != 0) {
...
int main(int argc, char *argv[])
{
    ...
    platform_init(&wifimgr->ctrl.handle);
    ...
    bus_get_vap_init_parameter(WIFI_DEVICE_MODE, &wifimgr->ctrl.network_mode);
    if (start_wifimgr() != 0) {
