...
TEST(WifiApi, GetApAssociatedDeviceDiagnosticResultTruncated)
{
    ...
    // note: this expected string is wrong. the platform mocks set up a devarray with three entries, but the function currently overwrites all previous content with
...
TEST(WifiApi, GetApAssociatedDeviceDiagnosticResultFull)
{
    wifi_ctrl_t *ctrl = (wifi_ctrl_t *)get_wifictrl_obj();
    wifi_bus_desc_t *bus_desc = get_bus_descriptor();
    bus_desc->bus_event_publish_fn=mock_bus_event_publish;
    ...
    // this test will fail until the code is corrected. note that this depends on a possible typo in process_wifiapi_command that creates
    // the result buffer with size 10024 instead of 1024.
    const char * expected = "\
